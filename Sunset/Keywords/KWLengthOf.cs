...
namespace Sunset.Keywords {
	...
	public class KWLengthOf : Keyword {
		...
		public override KeywordResult execute (Parser sender,String[] @params) {
			...
			if (@params.Length!=1)
				throw new ParsingError("Expected 1 array parameter for \""+constName+"\", got "+@params.Length.ToString()+" parameters",sender);
			
			VarType type=sender.pushValue(@params[0]).Item2;
			if (type!=VarType.NATIVE_ARRAY)
				throw new ParsingError("Expected an array as a parameter of \""+constName+"\", but got \""+type.ToString()+'"',sender);
